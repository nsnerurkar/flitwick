...
{
    char buf[7];
    sprintf_s(buf, "%02d%02d%02d", hour, minute, second);
    std::string str(buf);
    return(str);
}
...
std::string datetime::getDT()
{
    if (!year || !month || !day)
        return("");
    char buf[15];
    sprintf_s(buf, "%04d%02d%02d%02d%02d%02d", year, month, day,hour,minute,second);
    std::string str(buf);
    return(str);
}
...
d10lis::d10lis() :db("D10.db", "D10Result")
{
...
void d10lis::processData(d10Message* pMsg)
{
    ...
    switch(pMsg->getType())
    {
        ...
        case RESULT_MSG:
			...
			db.insertValues(rxCurrOrder, sProp, dynamic_cast<d10ResultRecordMessage*>(pMsg)->MeasVal);
            break;
    }
}
...
void d10lis::calcCheckSum(const char* buf, int size, unsigned char* high, unsigned char* low)
{
	...
	sprintf_s(strChkSum, "%.2X", checksum);
	*high = strChkSum[0];
	*low = strChkSum[1];
}
